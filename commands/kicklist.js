...
module.exports = {
	...
	async execute(message) {
		...
		await message.guild.members.fetch().then(fetchedMembers => {
			...
			const filteredLeft = left.filter(member => !member.roles.cache.some(r => r.name === 'Honorary Access') && !member.roles.cache.some(r => r.name === 'Cayde\'sMafia'));
			const visitorList = visitor.map(member => member.toString()).join(', ');
			const inactiveList = inactive.map(member => member.toString()).join(', ');
			const hiatusList = hiatus.map(member => member.toString()).join(', ');
			const missedList = missed.map(member => member.toString()).join(', ');
			const leftList = filteredLeft.map(member => member.toString()).join(', ');
			...
			const kicklist = new Discord.MessageEmbed()
				...
				.addFields([
					...
					{ name: `__**Left the Community**__ (${filteredLeft.size})`, value: leftList ? leftList : 'Everyone is a member.' },
